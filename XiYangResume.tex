...
\item
	...
	\begin{itemize}
                ...
                workloads with fine-grain feedback information from SHIM
...
\item
  ...
  \begin{itemize}
    \resitem{SHIM is an open source high frequency continuous profiler. I designed,
...
\item
  \ressubheadingTwo{Jikes RVM on
    OpenJDK}{\texttt{\href{http://hg.code.sourceforge.net/p/jikesrvm/code/shortlog/1c39f36bc842}{\color{blue}
        RVM-549-OpenJDK}}}    
  \begin{itemize}
    \resitem{Jikes RVM is not able to run many new critical applications
      because it is using the GNU Classpath library but not the state-of-the-art
      one OpenJDK library. This project ports the OpenJDK library to Jikes RVM.}
  \end{itemize}
\item
  \ressubheadingTwo{RTEMS on the Cortex-M3 CPU of OMAP4 SoC}{\texttt{\color{blue} https://github.com/yangxi/omap4m3}}
  \begin{itemize}
    \resitem{The OMAP4 SoC is an interesting platform for researchers who are interested in
      heterogeneous systems because it has two big Cortex-A9
      cores and two small Cortex-M3 cores. However, because there is no open
      source RTOS running on the small cores, it is hard to prototype ideas on the
